import 'package:auto_route/auto_route.dart';
import 'package:flutter/material.dart';
import 'package:meme_generator/common/widgets/di_scope/di_scope.dart';
import 'package:meme_generator/features/{{name.snakeCase()}}/data/repositories/{{name.snakeCase()}}_repository.dart';
import 'package:meme_generator/features/{{name.snakeCase()}}/di/{{name.snakeCase()}}_scope.dart';
import 'package:meme_generator/features/{{name.snakeCase()}}/presentation/screens/{{name.snakeCase()}}/{{name.snakeCase()}}_screen.dart';

/// {@template {{name.snakeCase()}}_flow.class}
/// Flow point to feature {{name.PascalCase()}}
/// {@endtemplate}
@RoutePage()
class {{name.pascalCase()}}Flow extends StatelessWidget implements AutoRouteWrapper {
  /// {@macro {{name.snakeCase()}}_flow.class}
  const {{name.pascalCase()}}Flow({super.key});

  @override
  Widget wrappedRoute(BuildContext context) {
    const repository = {{name.pascalCase()}}Repository();

    return DiScope<I{{name.pascalCase()}}Scope>(
      factory: () => {{name.pascalCase()}}Scope(repository),
      dispose: (scope) => scope.dispose(),
      child: this,
    );
  }

  @override
  Widget build(BuildContext context) {
    return const {{name.pascalCase()}}Screen();
  }
}
